...
def main() -> None:
    ...
    r, p = stats.pearsonr(sub["ess_total"], sub["ahi_sum_stages"])
    ok = abs(r - (-0.305)) < 0.03
    print(f"  [{'PASS' if ok else 'FAIL'}] within ESS>=8 (n={len(sub)}):        r={r:.3f}  p={p:.4f}  (thesis r=-0.305, p=0.138)")
    r, p = stats.pearsonr(inc["ess_total"], inc["ahi_sum_stages"])
    ...
    for label, col, rw in [
        ...
        ("arousal x AHI total", "ahi_sum_stages", 0.358),
    ]:
    ...
    pairs = [
        ("ODI x AHI total", "odi", "ahi_sum_stages", 0.570),
        ("ODI x SpO2 REM", "odi", "spo2_rem_mean", -0.123),
        ("ODI x SpO2 non-REM", "odi", "spo2_nonrem_mean", -0.057),
        ("SpO2 REM x AHI total", "spo2_rem_mean", "ahi_sum_stages", -0.358),
        ("SpO2 non-REM x AHI total", "spo2_nonrem_mean", "ahi_sum_stages", -0.271),
    ]
